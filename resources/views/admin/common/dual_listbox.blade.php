					...
					@if(isset($first) && count($first)>0)
						@foreach($first as $firstItem)
							<li
								...
								data-id="{{$firstItem->id}}"><i
									class="{{config('admin.settings.icon.'.$firstItem->status)}}"
									title="{{$firstItem->status}}"></i> {{$firstItem->name}} - {{$firstItem->email}}
							</li>
						@endforeach
					@endif
				</ul>
					...
					@if(isset($second) && count($second)>0)
						@foreach($second as $secondItem)
							<li
								id="item{{$firstItem->id}}"
								class="list-group-item"
								data-id="{{$secondItem->id}}"{{$secondItem->name}}>  <i
									class="{{config('admin.settings.icon.'.$firstItem->status)}}"
									title="{{$firstItem->status}}"></i>
								- {{$secondItem->email}}<input type="hidden"
								                                                       value="{{$firstItem->id}}"
								                                                       name="partners[]"/></li>
